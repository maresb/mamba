    ...
    PackageCacheData::has_valid_extracted_dir(const specs::PackageInfo& s, const ValidationParams& params)
    {
        ...
        if (fs::exists(extracted_dir))
        {
            ...
            if (fs::exists(repodata_record_path))
            {
                ...
                // CRITICAL: This check must happen AFTER all other validations to avoid being
                // overwritten by checksum validation which can set valid=true.
                if (valid && repodata_record.contains("timestamp") && repodata_record["timestamp"] == 0
                    && repodata_record.contains("license") && repodata_record["license"] == "")
                {
