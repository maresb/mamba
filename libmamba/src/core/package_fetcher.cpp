...
namespace mamba
{
    ...
    void PackageFetcher::write_repodata_record(const fs::u8path& base_path) const
    {
        ...
        //
        // When URL specs go through the solver (not --explicit), they create PackageInfo via
        // make_package_info() which sets defaulted_keys = ["_initialized"] (trusting all fields).
        // However, the solvable may not have complete metadata (license, timestamp), leaving
        // stub values (0, "") that would fail the healing check later.
        //
        // Detection: timestamp==0 AND license=="" is the corruption signature (very rare for
        // legitimate packages). When detected with only _initialized in defaulted_keys, treat
        // as URL-derived and erase stub fields.
        //
        // This is safe because:
        // - Channel repodata virtually always has license and timestamp
        // - The signature combination is extremely rare for legitimate packages
        ...
        const bool has_stub_signature = (m_package_info.timestamp == 0
                                         && m_package_info.license.empty());
