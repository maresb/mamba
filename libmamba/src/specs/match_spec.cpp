...
namespace mamba::specs
{
    ...
    auto MatchSpec::parse(std::string_view str) -> expected_parse_t<MatchSpec>
    {
        ...
        // or with sha256 prefix like
        // https://conda.anaconda.org/conda-forge/linux-64/pkg-6.4-bld.conda#sha256:abcd...
            ...
            if (has_archive_extension(url))
            {
                return MatchSpec::parse_url(url).transform(
                    [&](MatchSpec&& ms) -> MatchSpec
                    {
                        ...
                        {
                            auto sha256_hash = hash.substr(7);
                            if (sha256_hash.size() == 64 && is_hash(sha256_hash))
                            {
                                ms.set_sha256(std::string(sha256_hash));
                            }
                        }
...
) const -> format_context::iterator
{
    ...
    if (const auto& chan = spec.channel(); chan.has_value() && chan->is_package())
    {
        ...
        if (const auto& sha256 = spec.sha256(); !sha256.empty())
        {
            out = fmt::format_to(out, "{}sha256:{}", MatchSpec::url_md5_sep, sha256);
        }
        else if (const auto& md5 = spec.md5(); !md5.empty())
        {
