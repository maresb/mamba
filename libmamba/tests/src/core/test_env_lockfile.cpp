...
namespace mamba
{
    namespace
    {
        ...
        /**
         * EXPECTED FAILURE: Trusted lockfile dependencies should NOT be in defaulted_keys
         *
         * PURPOSE: Verify that when a conda lockfile provides dependencies AND has
         * a sha256 hash (indicating trustworthy metadata), "depends" is NOT listed
         * in defaulted_keys.
         *
         * TRUST INDICATOR: sha256 presence in the lockfile entry. Lockfiles
         * generated by conda-lock using a healthy micromamba include sha256.
         * Lockfiles generated during the v2.1.1-v2.4.0 bug period may have
         * corrupted metadata; missing sha256 is a corruption indicator. When
         * sha256 is absent, dependency metadata should be treated as untrusted
         * and remain in defaulted_keys so index.json can provide correct values.
         *
         * MOTIVATION: The conda lockfile parser (env_lockfile_conda.cpp):
         * 1. Parses hash (sha256 available at line 48-51)
         * 2. Calls from_url() → defaulted_keys includes "depends" and "constrains"
         * 3. Copies defaulted_keys from from_url() result (line 79)
         * 4. Populates dependencies from lockfile data (lines 82-89)
         *
         * After step 4, "depends" has real values from the lockfile but
         * defaulted_keys still lists it as defaulted. For trusted lockfiles
         * (sha256 present), the fix should remove "depends" from defaulted_keys.
         *
         * CURRENT BEHAVIOR (BUG): "depends" is always in defaulted_keys
         * regardless of sha256 presence, causing lockfile dependencies to be
         * silently replaced by index.json values in write_repodata_record().
         *
         * EXPECTED BEHAVIOR: When sha256 is present and the lockfile provides
         * dependencies, "depends" should be removed from defaulted_keys.
         *
         * Related: https://github.com/mamba-org/mamba/issues/4095
         */
        TEST_CASE("env-lockfile trusted lockfile dependencies not in defaulted_keys")
        {
            auto contains = [](const std::vector<std::string>& v, const std::string& val) -> bool
            { return std::find(v.begin(), v.end(), val) != v.end(); };

            // good_one_package-lock.yaml has sha256 AND dependencies:
            ...
            for (const auto& pkg : packages)
            {
                ...
                // Precondition: this lockfile has sha256 (trusted metadata)
                REQUIRE_FALSE(pkg.info.sha256.empty());

                // _initialized must be present
                REQUIRE(contains(pkg.info.defaulted_keys, "_initialized"));

                // For trusted lockfiles with dependencies, "depends" should NOT be
                // in defaulted_keys (the lockfile provided real values).
