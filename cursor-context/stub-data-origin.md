...
The stub values (`timestamp=0`, `license=""`, `build_number=0`) come from **C++ struct default member initializers** in `PackageInfo`. They are set when the struct is constructed and never overwritten because `from_url()` cannot extract these values from a URL.
...
## Key Insight: The Problem is NOT in libsolv

The stub values don't magically appear in libsolv. They are:

1. **Set when `PackageInfo` is constructed** via C++ default member initializers
2. **Never overwritten** because `from_url()` can't extract license/timestamp from a URL
3. **Copied to libsolv** by `set_solvable()` - libsolv is just storing what it's given
4. **Copied back** by `make_package_info()` - it reads what libsolv stored

The real problem is that `defaulted_keys` is lost during the libsolv round-trip, so `write_repodata_record()` doesn't know which fields have stub values.

## The Fix

The fix detects the "corruption signature" (`timestamp==0 AND license==""`) combined with `defaulted_keys.size()==1` to identify solver-path URL packages, then erases the stub fields before merging with `index.json`.
